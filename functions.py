...
def nb_input_prediction(text):

    text = pd.Series(text)
    text = preprocess(text)
    text = [text[0],]
    # to use this function we will need to define vectoriser first 
    vectoriser = pickle.load(open("Web_Resource/tdf_vectorizer", "rb"))
    text = vectoriser.transform(text)
    model = pickle.load(open("Web_Resource/model/nb_model.bin", "rb"))
...
def rf_input_prediction(text):
    ...
    vectoriser = pickle.load(open("Web_Resource/tdf_vectorizer", "rb"))
    text = vectoriser.transform(text)
    model = pickle.load(open("Web_Resource/model/rf_model.bin", "rb"))
    prediction = model.predict(text)
    prediction = prediction[0]
...
def svm_input_prediction(text):
    ...
    vectoriser = pickle.load(open("Web_Resource/tdf_vectorizer", "rb"))
    text = vectoriser.transform(text)
    model = pickle.load(open("Web_Resource/model/svm_model.bin", "rb"))
    prediction = model.predict(text)
    prediction = prediction[0]
